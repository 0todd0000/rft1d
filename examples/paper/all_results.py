...
[ax.text(-0.15, 0.5, r'$z$', size=24, transform=ax.transAxes, rotation=90, va='center')  for ax in (ax0,ax3)]
### panel labels:
...
### add a legend:
ax.plot([0,1],[1000,1000], 'k--', label=r'$EC$  (Hasofer, 1978)')
ax.plot([0,1],[1000,1000], 'k-', label=r'$HC$  (Worsley, 1995)')
...
print( '   u = scipy.optimize.fmin(objective_fn, x0) --> u = %.3f' %u.ravel()[0])
print
...
ax.text(-0.14, 0.5, r'$z^*$', size=20, transform=ax.transAxes, va='center', rotation=90)
ax.set_ylim(2.5, 5)
plt.show(block=False)
...
plt.figure(11)
colors  = ['b', 'g', 'r', 'orange']
labels  = [r'$u$ = %.1f'%h for h in heights]
...
ax.set_ylabel(r'$P(k_{max}) > x$', size=16)
ax.set_ylim(0, 0.25)
plt.show(block=False)
...
for color,p,p0,u in zip(colors,P,P0,heights):
    ax.plot(K0, p,  'o', color=color, markersize=5)
    ax.plot(K0, p0, '-', color=color, label=r'$u$ = %.1f'%u)
### legend:
ax.plot([0,1],[10,10], 'k-', label='Theoretical')
ax.plot([0,1],[10,10], 'ko-', label='Simulated', markersize=5)
ax.legend()
### axis labels:
ax.set_xlabel(r'$k_\mathrm{min}$', size=16)
ax.set_ylabel(r'$P(c | k_\mathrm{min}) >= 2$', size=16)
ax.set_ylim(0, 0.08)
plt.show(block=False)
